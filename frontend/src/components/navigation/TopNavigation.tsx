...
export const TopNavigation: React.FC<TopNavigationProps> = React.memo(({
  ...
  userName, userEmail, userDepartment, userInitials, userActions,
  notifications, onNotificationClick, onMarkAllNotificationsRead, onViewAllNotifications,
  className,
}) => (
  <div className={cn(styles.bar, className)}>
    <div className={styles.left}>
      <button type="button" className={styles.logoBtn} onClick={() => onNavigate('/home')} aria-label="Zur Startseite">
        <Logo size="sm" />
      </button>
      <nav className={styles.nav} aria-label="Hauptnavigation">
        {modules.filter((m) => m.visible !== false).map((mod) => (
          <button
            key={mod.id}
            type="button"
