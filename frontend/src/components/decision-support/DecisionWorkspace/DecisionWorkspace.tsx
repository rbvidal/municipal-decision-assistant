      ...
      <WorkspaceSection title="Zusammenfassung">
        <Panel>
          ...
          <div className={styles.metaRow}>
            ...
            <Badge status="info">
              Phase: {data.workflow.step}/{data.workflow.totalSteps}
            </Badge>
          </div>
        </Panel>
      </WorkspaceSection>
            ...
            <div key={step.id} className={styles.reasoningStep}>
              <div className={`${styles.stepDot} ${styles[`step-${step.status}`]}`}>
                {step.status === "completed" ? (
                  ...
                  <span className={styles.dotBlank} />
                )}
              </div>
        ...
        <Panel>
          <div className={styles.confidenceGrid}>
            ...
            <div className={styles.confItem}>
              ...
              <span className={styles.confValue}>
                {Math.round(data.confidence.coverage * 100)}%
              </span>
            ...
            <div className={styles.confItem}>
              ...
              <span className={styles.confValue}>
                {Math.round(data.confidence.ruleCompleteness * 100)}%
              </span>
              <ProgressIndicator
                value={Math.round(data.confidence.ruleCompleteness * 100)}
                size="sm"
              />
            </div>
          </div>
          {data.confidence.missingEvidence.length > 0 && (
            <Alert
              ...
              description={data.confidence.missingEvidence.join(", ")}
            />
          )}
        </Panel>
      </WorkspaceSection>
      ...
      <WorkspaceSection title="Empfehlung">
        <Panel>
          <div className={styles.recommendationHeader}>
            <Badge
              status={
                data.recommendation.action === "APPROVE"
                  ? "success"
                  : data.recommendation.action === "REJECT"
                    ? "error"
                    : "warning"
              }
            >
              {data.recommendation.action === "APPROVE"
                ...
                : data.recommendation.action === "REJECT"
                  ? "ABLEHNEN"
                  : data.recommendation.action === "REVISE"
                    ? "ÜBERARBEITEN"
                    : "INFO ANFORDERN"}
            </Badge>
          </div>
          ...
          {data.recommendation.manualReviewRequired && (
            <Alert
              ...
              description="Dieser Vorgang erfordert eine manuelle Prüfung durch den Sachbearbeiter."
            />
          )}
          <div className={styles.mt3}>
