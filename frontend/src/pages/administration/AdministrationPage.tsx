...
import {
  Panel, StatCard, Badge, Button, ProgressIndicator,
  PropertyGrid, ActionToolbar, EmptyState,
} from '../../components/common';
import { ConfirmDialog } from '../../components/interaction';
import {
  mockSystemHealth, mockBackgroundJobs, mockAuditLogs, mockDepartments, ADMIN_TABS,
...
const JOB_STATUS: Record<string, 'success' | 'info' | 'error' | 'neutral'> = {
  Completed: 'success', Running: 'info', Failed: 'error', Queued: 'neutral',
};
...
export const AdministrationPage: React.FC = React.memo(() => {
  ...
  const jobColumns: DataTableColumn<BackgroundJob>[] = useMemo(() => [
    { key: 'name', header: 'Auftrag', render: (j) => <span className={styles.jobName}>{j.name}</span> },
    { key: 'status', header: 'Status', render: (j) => <Badge status={JOB_STATUS[j.status] ?? 'neutral'}>{j.status}</Badge> },
    { key: 'progress', header: 'Fortschritt', render: (j) => <ProgressIndicator value={j.progress} max={100} size="sm" status={j.status === 'Failed' ? 'error' : j.status === 'Completed' ? 'success' : 'info'} /> },
    { key: 'startedAt', header: 'Gestartet', render: (j) => <span className={styles.monoCell}>{j.startedAt}</span> },
    { key: 'duration', header: 'Dauer', render: (j) => <span className={styles.monoCell}>{j.duration}</span> },
  ], []);

  const auditColumns: DataTableColumn<AuditLogEntry>[] = useMemo(() => [
    { key: 'timestamp', header: 'Zeitstempel', render: (l) => <span className={styles.monoCell}>{l.timestamp}</span> },
    { key: 'user', header: 'Benutzer', render: (l) => <span className={styles.monoCell}>{l.user}</span> },
    { key: 'action', header: 'Aktion', render: (l) => <span className={styles.monoCell}>{l.action}</span> },
    { key: 'target', header: 'Zielobjekt', render: (l) => <span className={styles.monoCell}>{l.target}</span> },
  ...
  const deptColumns: DataTableColumn<DepartmentConfig>[] = useMemo(() => [
    { key: 'name', header: 'Dezernat', render: (d) => <span className={styles.jobName}>{d.name}</span> },
    { key: 'shortCode', header: 'Kürzel', render: (d) => <Badge status="info" variant="pill">{d.shortCode}</Badge> },
    { key: 'activeUsers', header: 'Aktive Benutzer', render: (d) => <span className={styles.monoCell}>{String(d.activeUsers)}</span> },
    { key: 'totalCases', header: 'Vorgänge', render: (d) => <span className={styles.monoCell}>{String(d.totalCases)}</span> },
  ], []);
  ...
  return (
    <AppShell
      ...
      <div className={styles.page}>
        ...
        {activeTab === 'overview' && (
          <>
            <div className={styles.statsGrid}>
              <StatCard label="CPU" value={mockSystemHealth.cpuPercent} percentage={mockSystemHealth.cpuPercent} status="info" />
              <StatCard label="Speicher" value={mockSystemHealth.memoryPercent} percentage={mockSystemHealth.memoryPercent} status="warning" />
              <StatCard label="Festplatte" value={mockSystemHealth.diskPercent} percentage={mockSystemHealth.diskPercent} status="success" />
            ...
            <div className={styles.overviewGrid}>
              ...
              <Panel title="Schnellaktionen">
                <ActionToolbar actions={[
                  { id: 'reindex', label: 'Re-Indizierung starten', onClick: () => {}, variant: 'primary' },
                  { id: 'clearcache', label: 'Cache leeren', onClick: () => {}, variant: 'secondary' },
                  { id: 'restart', label: 'Dienst neustarten', onClick: () => {}, variant: 'danger' },
                ]} />
              </Panel>
            </div>
          </>
        ...
        {activeTab === 'audit' && (
          <Panel title="Audit-Protokoll" headerAction={
            <Button variant="ghost" size="sm" onClick={() => setConfirmDelete('audit')}>Protokoll löschen</Button>
          }>
            {auditLogs.length === 0 ? <EmptyState title="Keine Audit-Einträge" /> : (
              ...
              <div>
                <div className={styles.settingsRow}>
                  <div>
                    ...
                    <p className={styles.settingsDesc}>Neue Dokumente werden automatisch indiziert.</p>
                  </div>
                  <input type="checkbox" defaultChecked className={styles.settingsInput} style={{ width: 'auto' }} />
                </div>
                <hr className={styles.settingsDivider} />
                <div className={styles.settingsRow}>
                  <div>
                    <span className={styles.settingsLabel}>Embedding-Modell</span>
                    <p className={styles.settingsDesc}>Verwendetes Modell für Vektoreinbettungen.</p>
                  </div>
                ...
                <hr className={styles.settingsDivider} />
                <Button variant="primary" size="sm">Einstellungen speichern</Button>
              </div>
            </div>
          </Panel>
        )}
