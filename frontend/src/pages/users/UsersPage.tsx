    ...
    () => [
      {
        ...
        render: (u) => (
          <div>
            <div className={styles.userName} onClick={() => setSelectedUser(u)}>
              {u.name}
            </div>
      ...
      },
      {
        key: "role",
        header: "Rolle",
        render: (u) => <Badge status={ROLE_COLORS[u.role] ?? "neutral"}>{u.role}</Badge>,
      },
      {
        key: "department",
        header: "Dezernat",
        render: (u) => <span className={styles.monoCell}>{u.department}</span>,
      },
      {
        key: "status",
        header: "Status",
        render: (u) => <Badge status={STATUS_COLORS[u.status] ?? "neutral"}>{u.status}</Badge>,
      },
      {
        key: "lastAccess",
        header: "Letzter Zugriff",
        render: (u) => <span className={styles.monoCell}>{u.lastAccess}</span>,
      },
      {
        key: "actions",
        header: "",
  ...
  return (
    ...
    >
      ...
      <ConfirmDialog
        ...
        description={
          confirmToggle
            ? `${confirmToggle.name} (${confirmToggle.email}) wird ${confirmToggle.status === "Aktiv" ? "deaktiviert" : "aktiviert"}.`
            : ""
        }
